...
public class BuildingGridController : MonoBehaviour
{
    ...
    public GameObject TownStatHolder;
    private TownStatHolder TownStatHolderScript;
    ...
    public string DexterityPref = "Dexterity";
    public string IntellectPref = "Intellect";
    public string StrengthPref = "Strength";
    public string StaminaPref = "Stamina";
    ...
    void Start()
    {
        ...
        // load all buildings from memory
        loadBuildings();
        ...
        for (int i = 0; i < goldInInventory.Count; i++)
        {
            ...
            goldInInventory[i].RemoveItem();
        }
        // inventory.Clear();

        // update character bonuses
        TownStatHolderScript = TownStatHolder.GetComponent<TownStatHolder>();
    ...
    #region - Basic Building Mechanics -
    ...
    {
        if (Input.GetMouseButtonDown(0) && gridObjects[closestX, closestY] == null) // place onto grid when user clicks
        {
    ...
    #region - Advanced Building Mechanics -
    ...
    public void UpdateCharacterBonuses()
    {
        ...
        // update bonuses in memory
        PlayerPrefs.SetInt(DexterityPref, attributes[dexterity]);
        PlayerPrefs.SetInt(IntellectPref, attributes[intellect]);
        PlayerPrefs.SetInt(StrengthPref, attributes[strength]);
        PlayerPrefs.SetInt(StaminaPref, attributes[stamina]);
        TownStatHolderScript.UpdateTownStats(attributes[dexterity], attributes[intellect], attributes[strength], attributes[stamina]);
    }
        ...
        while (i < gridRows)
        {
            ...
            while (j < gridRows)
            {
                ...
                if (type >= (int)Building.Tavern && type <= (int)Building.Bank)
                {
                    ...
                    buildingScript.InitializeInfo();
                    gridObjects[i, j] = selectedBuilding;
                    UpdateAdjacentBuildings(i, j);
    ...
    public void ClearMemory()
    {
        ...
        currentGold = 1000;
        PlayerPrefs.SetInt(GoldPref, currentGold);
        PlayerPrefs.SetInt(DexterityPref, 0);
        PlayerPrefs.SetInt(IntellectPref, 0);
        PlayerPrefs.SetInt(StrengthPref, 0);
        PlayerPrefs.SetInt(StaminaPref, 0);
