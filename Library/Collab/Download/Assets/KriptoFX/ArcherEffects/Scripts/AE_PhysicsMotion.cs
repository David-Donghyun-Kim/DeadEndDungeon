...
public class AE_PhysicsMotion : MonoBehaviour
{
    ...
    [SerializeField]
    ParticleSystem darkMagicParticle;
    [SerializeField]
    ParticleSystem darkMagicFog;
    [SerializeField]
    ParticleSystem fireMagicParticle;
    [SerializeField]
    ParticleSystem fireExplosion;
    [SerializeField]
    ParticleSystem fireExplosionBack;
    [SerializeField]
    ParticleSystem iceMagicParticle;
    [SerializeField]
    ParticleSystem iceMagicExplosion;
    [SerializeField]
    AudioSource audioSource;

    public bool isDarkMagic = false;
    ...
    void OnEnable()
    {
        ...
        InitializeRigid();
        if (darkMagicParticle != null && darkMagicFog != null)
        {
            darkMagicParticle.Stop();
            darkMagicFog.Stop();
            isDarkMagic = true;
        }
        if (fireMagicParticle != null && fireExplosion != null && fireExplosionBack != null)
        {
            fireMagicParticle.Stop();
            fireExplosion.Stop();
            fireExplosionBack.Stop();
            isFireMagic = true;
        }
        if(iceMagicParticle != null && iceMagicExplosion != null)
        {
            iceMagicParticle.Stop();
            iceMagicExplosion.Stop();
            isIceMagic = true;
        }
    }
    ...
    void InitializeForce()
    {
        ...
        rigid.AddForce(cameraForward.normalized * (Speed + currentSpeedOffset), ForceMode);
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if(isFireMagic)
        {
            ...
            fireExplosion.Play();
            fireExplosionBack.Play();
            audioSource.Play();
        }
        if(isIceMagic)
        {
            iceMagicParticle.Play();
            iceMagicExplosion.Play();
            audioSource.Play();
        }
